...
def main():
    ...
    with tab1:
        ...
        search_sales = st.text_input("🔍 Cari PN atau Keterangan:", key="s_sales")
        if search_sales:
            ...
            if not_found:
                ...
                try:
                    requests.get(f"https://api.telegram.org/bot{st.secrets['TELEGRAM_TOKEN']}/sendMessage?chat_id={st.secrets['TELEGRAM_CHAT_ID']}&text=⚠️ Sales: {', '.join(not_found)}")
                except: pass
            ...
            if st.button("Proses Gambar"):
                with st.spinner("AI sedang membaca..."):
                    ...
                    if not_found:
                        st.error(f"Peringatan: Item tidak ditemukan: **{', '.join(not_found)}**")
                        try:
                            requests.get(f"https://api.telegram.org/bot{st.secrets['TELEGRAM_TOKEN']}/sendMessage?chat_id={st.secrets['TELEGRAM_CHAT_ID']}&text=⚠️ Sales via Gambar: {', '.join(not_found)}")
                        except: pass